...
void Paint_SetPixel(UWORD Xpoint, UWORD Ypoint, UWORD Color)
{
    ...
    if(Paint.Scale == 2){
        ...
        */
        ...
        // Might need to use Height here?
        UDOUBLE BitPlace = Y*Paint.Width + X;
        UWORD ByteAddr = BitPlace / 8;
        UWORD ByteOffs = 7 - BitPlace % 8;
        UBYTE ModByte = 1 << ByteOffs;
        if (Color == BLACK){
            Paint.Image[ByteAddr] = Paint.Image[ByteAddr] | ModByte;
        }
        else if (Color == WHITE){
            ModByte ^= 0b11111111;
            Paint.Image[ByteAddr] = Paint.Image[ByteAddr] & ModByte;
