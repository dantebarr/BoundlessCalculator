...
parameter:
******************************************************************************/
void LCD_1IN8_Display(UBYTE *Image)
{
    UWORD j;
    UWORD i;
    UBYTE nextBit = 0;
    ...
    DEV_Digital_Write(EPD_DC_PIN, 1);
    DEV_Digital_Write(EPD_CS_PIN, 0);
    UWORD row[LCD_1IN8.WIDTH];
    for (j = 0; j < LCD_1IN8.HEIGHT; j++) {
        for (i = 0; i < LCD_1IN8.WIDTH; i++){
            //Check bit value
            //Determine next bit value = (byte of current bit) >> (offset to bit shift to LSB) & 1 to extract LSB
            nextBit = Image[(j*LCD_1IN8.WIDTH + i)/8] >> (7 - (j*LCD_1IN8.WIDTH + i) % 8) & 1;
            //printf("%i", nextBit);
            if (nextBit == 0){
                row[i] = 0xFFFF; //WHITE
                //Uncomment to print screen representation in terminal
                //printf("-");
            }
            else{
                row[i] = 0x0000; //BLACK
        ...
        //Uncomment to print screen representation in terminal
        //printf("\n");
        DEV_SPI_Write_nByte((uint8_t *)row, LCD_1IN8.WIDTH*2);
    }
    DEV_Digital_Write(EPD_CS_PIN, 1);
    LCD_1IN8_SendCommand(0x29);
}
